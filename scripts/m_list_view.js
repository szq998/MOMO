...
class MemoryListView {
    ...
    async changeDescription(sender, indexPath, data) {
        let desc = await this.callBack.inputDescription()
        if (desc) {
            let newData = data
    ...
    // Todo: change to async function, have problem here
    changeContent(sender, indexPath, data) {
        let updateListDataWithContentInfo = newContentInfo => {
            if (data.contentInfo.category == newContentInfo.category) {
                ...
                if (this.callBack.getCurrentCategory() != null) sender.delete(indexPath)
            }
        }
